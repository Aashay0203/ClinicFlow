...
export async function updateHealthProfileAI(userId) {
    try {
        console.log(`[Health Profile] Starting AI extraction for user ${userId}`);
        ...
        if (!consolidatedData) {
            throw new Error(
                "Gemini returned null consolidation for user " + userId
            );
        }
        ...
        console.error(
            `[Health Profile] Error updating profile for user ${userId}:`,
            error.message
        );
        throw error;
    }
...
export async function updateHealthProfileManual(userId, userProvidedData) {
    try {
        console.log(`[Health Profile] Saving user data for user ${userId}`);
